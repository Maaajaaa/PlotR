...
void coordinateSystem(unsInt type, unsInt xLenghtPerSide, unsInt yLenghtPerSide, unsInt notchesSpace)
{
    ...
    case 0: //only quadrant 1 |_
        ...
        if(notchesSpace > 0)    //with notches
            for(unsInt i = 0; i < floor(xLenghtPerSide / notchesSpace); i++)
            {
                xAxis.move(notchesSpace);    //use notches
                drawHorizontalNotch();
            }
        else
            xAxis.move(xLenghtPerSide);
    break;
    default:
        break;
    }
...
void drawHorizontalNotch()
{
    xAxis.wakeUp();
    down();
    xAxis.move(notchLenghtPerSide/XmmSt);
    up();
    xAxis.move(-2*(notchLenghtPerSide/XmmSt));
    down();
    xAxis.move(-notchLenghtPerSide/XmmSt);
}
